...
export default function TopProductsCard() {
  ...
  const { data: products, isLoading } = useQuery({
    ...
    queryFn: async () => (await axiosInstance.get(`/api/analytics/top-products?range=${range}`)).data as Product[],
    staleTime: 1000 * 60 * 5,
  });
    ...
    <div className="w-full bg-white dark:bg-slate-950 rounded-[2.5rem] p-6 border border-gray-100 dark:border-slate-800 shadow-xl shadow-slate-200/40 dark:shadow-none flex flex-col relative h-[700px] transition-all duration-300 hover:shadow-2xl hover:shadow-slate-200/60 hover:-translate-y-1">
      ...
      <div className="flex justify-between items-center mb-6 z-20">
        ...
        <div className="scale-90 origin-right">
          <CustomSelect 
             value={range} 
             onChange={(val) => { setRange(val); setCurrentIndex(0); }} 
              ...
              <div className="mt-6 pt-5 border-t border-gray-100 dark:border-slate-800 flex items-center justify-between">
                 ...
                 <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                        <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Total Sold</span>
                        <span className="text-lg font-extrabold text-slate-800 dark:text-white leading-none">
                            {currentProduct.totalSold} <span className="text-xs font-medium text-slate-400">Units</span>
                        </span>
                    </div>
                 </div>
