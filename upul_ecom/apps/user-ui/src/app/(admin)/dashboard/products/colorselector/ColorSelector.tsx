  ...
  return (
    <div className="space-y-4 p-4 border border-gray-100 rounded-xl bg-white shadow-sm">
      <div className="flex justify-between items-center">
        <label className="text-sm font-semibold text-gray-800">Available Colors</label>
        <button
          type="button"
          onClick={() => setIsAdding(!isAdding)}
          className={`flex items-center gap-1 text-xs font-medium px-2 py-1 rounded-md transition ${
            isAdding ? 'bg-red-50 text-red-600' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
          }`}
        >
      ...
      <div className="flex flex-wrap gap-4">
        {isLoading ? (
          <p className="text-xs text-gray-400 animate-pulse">Loading palette...</p>
        ) : (
          colors.map((color) => {
                ...
                >
                  {isSelected && (
                    <div className="absolute inset-0 flex items-center justify-center bg-black/10 rounded-full">
                      <Check size={18} className={parseInt(color.hexCode.replace('#',''), 16) > 0xffffff/2 ? 'text-black' : 'text-white'} />
                    </div>
                  )}
                </div>
                <span className={`text-[10px] uppercase tracking-wider font-medium ${isSelected ? 'text-black font-bold' : 'text-gray-500'}`}>
                  {color.name}
                </span>
              </div>
            );
