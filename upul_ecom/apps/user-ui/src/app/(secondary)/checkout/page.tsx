...
export default function CheckoutPage() {
  ...
  // ✅ UPDATED: take coupon stuff from store
  const {
    items,
    couponCode,
    discountAmount,
  ...
  // ✅ UPDATED: final total comes from store (subtotal - couponDiscount) + shipping
  const grandTotal = useMemo(() => {
  const totalAfterDiscount = subtotal - (discountAmount || 0);
  // Ensure we don't go below zero, then add shipping
          ...
          <section className="mb-10 md:mb-12">
            ...
            <div
              ...
              className={`border-2 rounded-lg p-4 md:p-5 cursor-pointer transition-all flex justify-between items-center ${
                paymentSelected ? 'border-black bg-gray-50' : paymentError ? 'border-red-500 bg-red-50' : 'border-gray-200 hover:border-gray-300'
              }`}
            >
              <div>
              ...
              <div
                className={`w-5 h-5 md:w-6 md:h-6 rounded-full border-2 flex items-center justify-center ${
                  paymentSelected ? 'border-black' : paymentError ? 'border-red-500' : 'border-gray-300'
                }`}
              >
