...
const checkoutSchema = z.object({
  email: z.string().email("Invalid email address").optional(), // Required for Guest
  ...
  saveAddress: z.boolean().default(false).optional(), // Only for User adding new address
});
...
export default function CheckoutPage() {
  ...
  // ✅ Make cart values safe on first render
  const cart = useCart();
  const items: CartItem[] = Array.isArray(cart?.items) ? cart.items : [];
  const rawSubtotal = cart?.subtotal; // might be undefined
  ...
  // ✅ Safe subtotal: use hook subtotal if valid, else compute from items
  ...
  const safeSubtotal = useMemo(() => {
    const n = Number(rawSubtotal);
    return Number.isFinite(n) ? n : computedSubtotal;
  }, [rawSubtotal, computedSubtotal]);
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 sticky top-4">
            ...
            <div className="border-t pt-4 space-y-2 text-sm">
              <div className="flex justify-between text-gray-600">
                <span>Subtotal</span>
                <span>LKR {safeSubtotal.toLocaleString()}</span>
              </div>
              <div className="flex justify-between text-gray-600">
                <span>Shipping</span>
