...
import { Search, ShoppingCart, User, Heart, Menu, X, Package, UserCircle } from 'lucide-react';
...
useEffect(() => {
  ...
  return (
    <>
      ...
      >
        <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isAtTop ? 'max-h-[200px] opacity-100' : 'max-h-0 opacity-0'}`}>
          ...
          <div className="max-w-8xl mx-auto px-4 md:px-5 h-16 md:h-20 flex items-center justify-between gap-4 md:gap-8 border-b">
            ...
            <div className="flex items-center gap-3 md:gap-5">
              <Link href={isLoggedIn ? "/profile" : "/login"} className="hidden sm:flex items-center gap-2 hover:text-red-600 transition-all text-gray-800">
              ...
              <Link href="/wishlist" className="relative text-gray-800 hover:text-red-600 transition-colors p-1">
                ...
                {wishlistItems.length > 0 && (
                  <span className="absolute top-0 right-0 bg-red-500 text-white text-[9px] w-4 h-4 rounded-full flex items-center justify-center font-bold">{wishlistItems.length}</span>
                )}
              </Link>
              ...
              <button onClick={toggleCart} className="relative text-gray-800 hover:text-black transition-colors p-1">
                ...
                {items.length > 0 && (
                  <span className="absolute top-0 right-0 bg-black text-white text-[9px] w-4 h-4 rounded-full flex items-center justify-center">{items.length}</span>
                )}
              </button>
            </div>
          </div>
        ...
        <nav className="bg-[#f2e6e6] border-b overflow-x-auto scrollbar-hide shadow-sm w-full">
